...
describe Health::CheckRun, type: :model do
  describe "validations" do
    subject { Health::CheckRun.new(health_check_name: "testerino", ran_at: Time.zone.now) }
  ...
  describe ".last_ran_at" do
    subject { Health::CheckRun.last_ran_at(MockHealthCheck, :example) }

    let!(:record1) { FactoryBot.create(:health_check_run, ran_at: Time.zone.yesterday) }
    let!(:record2) { FactoryBot.create(:health_check_run, ran_at: Time.zone.now) }
  ...
  describe ".to_health_check_name" do
    subject { Health::CheckRun.to_health_check_name(MockHealthCheck, :example) }
